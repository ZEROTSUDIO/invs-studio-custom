	...
	<div style="max-width: 640px;">
		...
		<div style="margin-bottom:22px;">
			...
			<div style="font-size:11px; color: var(--smoke); margin-top:4px;">
				Pesanan akan ditambahkan ke antrian dan dijadwalkan oleh admin.
			</div>
		</div>
		...
		<?php if ($this->input->get('alert') == 'no_customer'): ?>
			<div class="alert" style="background:rgba(248,113,113,0.1); border-color:rgba(248,113,113,0.3); color:#f87171; margin-bottom:16px;">
				⚠ &nbsp;Silakan pilih atau buat pelanggan sebelum mengirim.
			</div>
		<?php endif; ?>
			...
			<div class="panel">
					...
					<div class="form-group" id="customer-picker-group">
						...
						<div style="position:relative;">
							<input
								...
								class="form-input"
								placeholder="Ketik nama atau nomor telepon untuk mencari…"
								autocomplete="off"
							>
							<!-- Dropdown results -->
						...
						<div id="new-customer-form" style="display:none; margin-top:12px; padding:14px; border:1px solid var(--ghost); background:rgba(255,255,255,0.02);">
							<div style="font-size:11px; color:var(--smoke); margin-bottom:10px; letter-spacing:0.1em; text-transform:uppercase;">Pelanggan Baru</div>
							<div class="form-group" style="margin-bottom:10px;">
								<input type="text" id="new-name" class="form-input" placeholder="Nama lengkap" style="margin-bottom:0;">
							</div>
							<div class="form-group" style="margin-bottom:10px;">
								<input type="text" id="new-phone" class="form-input" placeholder="Telepon / WhatsApp" style="margin-bottom:0;">
							</div>
							<div style="display:flex; gap:8px;">
								<button type="button" onclick="submitNewCustomer()" class="btn btn-primary" style="font-size:11px; padding:6px 14px;">Buat & Pilih</button>
								<button type="button" onclick="toggleNewCustomerForm(false)" class="btn btn-ghost" style="font-size:11px; padding:6px 14px;">Batal</button>
							</div>
				...
				<div class="form-group">
					...
					<div id="duration-box" style="display:none; padding:12px; background: rgba(232,160,32,0.08); border:1px solid rgba(232,160,32,0.25); margin-bottom:18px;">
						<div style="font-size:10px; color: var(--smoke);">Estimasi Waktu Produksi</div>
					...
					<!-- FILE -->
					<div class="form-group">
						<label class="form-label">Unggah File Desain</label>
...
function liveSearch(q) {
	fetch(SEARCH_URL + '?q=' + encodeURIComponent(q))
		...
		.then(data => {
			...
			dd.innerHTML = '';
			if (data.length === 0) {
				dd.innerHTML = '<div style="padding:10px 14px; font-size:11px; color:var(--smoke);">Tidak ada kecocokan — gunakan "+ Pelanggan baru" di bawah.</div>';
			} else {
				data.forEach(c => {
	...
	fetch(CREATE_URL, { method: 'POST', body: fd })
		...
		.then(data => {
			if (data.success) {
				...
				errEl.textContent = data.message || 'Gagal membuat pelanggan baru.';
				errEl.style.display = 'block';
			}
		});
}
...
function fetchSafeDeadline(mins) {
    ...
    hint.textContent = 'Menghitung tenggat waktu paling aman...';
    ...
    fetch(DEADLINE_URL + '?est_duration=' + mins)
        ...
        .then(data => {
            ...
            if (data.earliest_date) {
                ...
                hint.textContent = 'Berdasarkan antrian, tenggat aman paling cepat adalah ' + earliestSafeDate;
...
function checkDeadline() {
    ...
    if (input && earliestSafeDate && input < earliestSafeDate) {
        warn.innerHTML = `<b>Terlalu Cepat!</b> Sistem memproyeksikan selesai sekitar tanggal ${earliestSafeDate}. Menggunakan tanggal ini berisiko melewati tenggat.`;
