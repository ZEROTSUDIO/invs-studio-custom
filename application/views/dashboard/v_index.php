...
<div class="page active" id="page-admin-dashboard">
    ...
    <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:14px; margin-bottom:20px;">
        ...
        <div class="stat-card">
            ...
            <div class="stat-sub" style="color:var(--smoke);">
                <?php echo $stats->waiting; ?> menunggu · <?php echo $stats->scheduled; ?> terjadwal
            </div>
        </div>
        ...
        <div class="stat-card" style="--before-color: #4ade80;">
            ...
            <div class="stat-sub" style="color:#4ade80;">
                <?php echo $stats->in_progress > 0 ? '● Sedang berjalan' : 'Tidak ada produksi'; ?>
            </div>
        </div>
    ...
    <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-bottom:20px;">
        ...
        <a href="<?php echo base_url() . 'dashboard/new_order'; ?>" id="shortcut-new-order"
            ...
            <div>
                <div style="font-family:'Syne',sans-serif; font-weight:700; font-size:12px; color:var(--cream); letter-spacing:0.06em;">PESANAN BARU</div>
                <div style="font-size:10px; color:var(--smoke); margin-top:2px;">Buat & jadwalkan</div>
            </div>
        </a>
        ...
        <a href="<?php echo base_url() . 'dashboard/orders'; ?>" id="shortcut-orders"
            ...
            <div>
                <div style="font-family:'Syne',sans-serif; font-weight:700; font-size:12px; color:var(--cream); letter-spacing:0.06em;">SEMUA PESANAN</div>
                <div style="font-size:10px; color:var(--smoke); margin-top:2px;">Lihat & kelola</div>
            </div>
        </a>
        ...
        <a href="<?php echo base_url() . 'dashboard/schedule'; ?>" id="shortcut-schedule"
            ...
            <div>
                <div style="font-family:'Syne',sans-serif; font-weight:700; font-size:12px; color:var(--cream); letter-spacing:0.06em;">JADWAL</div>
                <div style="font-size:10px; color:var(--smoke); margin-top:2px;">Gantt · Antrian SJF</div>
            </div>
        </a>
        ...
        <a href="<?php echo base_url() . 'dashboard/customers'; ?>" id="shortcut-customers"
            ...
            <div>
                <div style="font-family:'Syne',sans-serif; font-weight:700; font-size:12px; color:var(--cream); letter-spacing:0.06em;">PELANGGAN</div>
                <div style="font-size:10px; color:var(--smoke); margin-top:2px;">Cari · tambah baru</div>
            </div>
        </a>
    ...
    <div style="display:grid; grid-template-columns: 1fr 360px; gap:16px;">
        ...
        <div class="panel">
            <div class="panel-header">
                <div class="panel-title">Pesanan Kerja Hari Ini</div>
            ...
            <?php if (empty($today_work)): ?>
                <div style="padding:32px; text-align:center; color:var(--smoke);">
                    ...
                    <div style="font-family:'Syne',sans-serif; font-size:13px;">Tidak ada jadwal produksi hari ini</div>
                    <div style="font-size:11px; margin-top:4px;">Antrian bersih untuk hari ini</div>
                </div>
            <?php else: ?>
                <div style="padding:14px; display:flex; flex-direction:column; gap:10px;">
                    <?php foreach ($today_work as $job):
                        ...
                        $label_txt   = $is_done ? 'Selesai' : ($is_active ? '● Dalam Produksi' : '◌ Terjadwal');
                        $days_left   = days_until($job->deadline);
                        $dl_color    = ($days_left !== null && $days_left <= 0) ? '#f87171' : (($days_left !== null && $days_left <= 1) ? '#e8a020' : '#6b7280');
                        $dl_label    = ($days_left !== null) ? ($days_left < 0 ? abs($days_left) . 'hr terlambat' : ($days_left === 0 ? 'Batas hari ini' : $days_left . 'hr lagi')) : '';
                    ...
                    ?>
                        <div id="workorder-<?php echo $job->order_id; ?>"
                            ...
                            <div style="display:flex; align-items:baseline; gap:8px; margin-bottom:6px;">
                                ...
                                <div style="font-size:11px; color:var(--smoke);"><?php echo htmlspecialchars($job->category_name ?: 'Custom'); ?></div>
                            ...
                            <div style="display:flex; align-items:center; gap:8px; margin-top:4px;">
                                    ...
                                    <div style="margin-left:auto; display:flex; gap:6px; align-items:center;">
                                        <?php if ($job->schedule_status === 'scheduled'): ?>
                                            <form method="POST" action="<?php echo base_url('dashboard/update_status/' . $job->order_id . '/in_progress'); ?>" style="margin:0;">
                                                <button type="submit" onclick="return confirm('Mulai produksi <?php echo htmlspecialchars($job->order_code); ?>?')"
                                        ...
                                        <?php else: ?>
                                            <form method="POST" action="<?php echo base_url('dashboard/update_status/' . $job->order_id . '/done'); ?>" style="margin:0;">
                                                <button type="submit" onclick="return confirm('Tandai <?php echo htmlspecialchars($job->order_code); ?> sebagai Selesai?')"
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-content" style="bottom: 100%; top: auto; right: 0; margin-bottom: 5px;">
                                                <a href="<?php echo base_url('dashboard/edit_order/' . $job->order_id); ?>">✏ Edit Detail</a>
                                ...
                                <?php else: ?>
                                    <span style="margin-left:auto; font-size:10px; font-family:'Syne',sans-serif; color:#4ade80; letter-spacing:0.08em;">✓ SELESAI</span>
                                <?php endif; ?>
                            </div>
                            ...
                            <?php if ($is_active): ?>
                                ...
                                <div style="font-size:9px; color:var(--smoke); margin-top:3px; text-align:right;"><?php echo $progress; ?>% berjalan</div>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                </div>
        ...
        <div style="display:flex; flex-direction:column; gap:14px;">
            ...
            <?php if (!empty($deadline_alerts)): ?>
                <div class="panel" style="border-left:3px solid #f87171;">
                    <div class="panel-header">
                        <div class="panel-title" style="color:#f87171;">⚠ Peringatan Tenggat</div>
                        <span style="font-size:9px; color:#f87171; letter-spacing:0.1em;">BATAS ≤ 1 HARI</span>
                    </div>
                    <div style="padding:10px 14px; display:flex; flex-direction:column; gap:8px;">
                        <?php foreach ($deadline_alerts as $alert):
                            $d = days_until($alert->deadline);
                            $d_txt = $d < 0 ? abs($d) . 'hr terlambat!' : ($d === 0 ? 'Batas HARI INI' : 'Batas besok');
                        ...
                        ?>
                            <div style="display:flex; align-items:center; gap:10px; padding:8px 10px; background:rgba(248,113,113,0.07); border:1px solid rgba(248,113,113,0.25);">
                                ...
                                <a href="<?php echo base_url() . 'dashboard/orders'; ?>"
                                    ...
                                    onmouseout="this.style.background='transparent'">LIHAT</a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="panel" style="flex:1;">
                <div class="panel-header">
                    <div class="panel-title">Pratinjau Antrian</div>
                ...
                <?php if (empty($queue_preview)): ?>
                    <div style="padding:24px; text-align:center; color:var(--smoke); font-size:11px;">Antrian kosong</div>
                <?php else: ?>
                    <div style="padding:14px; display:flex; flex-direction:column; gap:10px;">
                        <?php foreach ($queue_preview as $item):
                            $q_active  = $item->status === 'in_progress';
                            $q_accent  = $q_active ? '#4ade80' : '#60a5fa';
                            $q_label   = $q_active ? '● Dalam Produksi' : '◌ Terjadwal';
                                ...
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:3px;">
                                    ...
                                    <?php if ($item->deadline && $dl_left !== null && $dl_left <= 2): ?>
                                        <div style="font-size:9px; color:<?php echo $dl_left <= 0 ? '#f87171' : '#e8a020'; ?>; padding:1px 5px; border:1px solid <?php echo $dl_left <= 0 ? '#f87171' : '#e8a020'; ?>;">
                                            <?php echo $dl_left <= 0 ? 'TERLAMBAT' : $dl_left . 'hr'; ?>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <?php if ($q_active): ?>
