...
function calcDurationDays(qty) {
	...
	if (val) val.textContent = days + (days === 1 ? " hari" : " hari");
	if (note)
		note.textContent = `Rentang: ${label} → ${days} hari. Posisi antrian sebenarnya ditentukan oleh SJF.`;
...
if (typeof window.calcDuration === "undefined") {
	window.calcDuration = function () {
		...
		if (total > 0) {
			if (box) box.style.display = "block";
			if (val) val.innerText = duration + " menit";
