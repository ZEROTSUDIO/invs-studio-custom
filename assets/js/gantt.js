...
(function () {
	...
	function renderHeader() {
		...
		for (var i = 0; i < n; i++) {
			...
			if (sun) html += '<br><span style="font-size:6px;opacity:0.7">OFF</span>';
	...
	function renderRows() {
		...
		data.forEach(function (s) {
			...
			if (dlPct !== null && dlPct >= 0 && dlPct <= 100) {
				...
				tick.title = 'Deadline: ' + fmtDate(s.deadline);
	...
	function renderMeta() {
		...
		var modeMap = { week: 'Week (7d)', '2weeks': '2 Weeks (14d)', month: 'Month (30d)' };
	...
	function showTooltip(e, s) {
		...
		t.innerHTML =
			...
			'<div class="gantt-tooltip-label">🕐 Start</div><div>' + fmtDateTime(s.start_date) + '</div>' +
			'<div class="gantt-tooltip-label" style="margin-top:4px;">🕐 End</div><div>' + fmtDateTime(s.end_date) + '</div>' +
			(s.deadline
				? '<div class="gantt-tooltip-label" style="margin-top:4px;">📅 Deadline</div><div style="color:#f87171;">' + fmtDate(s.deadline) + '</div>'
				: '') +
			'<div class="gantt-tooltip-divider"></div>' +
			'<div class="gantt-tooltip-stats">' +
				'<div><div class="gantt-tooltip-label">Qty</div><div style="font-weight:600;">' + s.qty + 'p</div></div>' +
				'<div><div class="gantt-tooltip-label">Est. Duration</div><div style="font-weight:600;">' + fmtDuration(s.est_duration) + '</div></div>' +
			'</div>';
